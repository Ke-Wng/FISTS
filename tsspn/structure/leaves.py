...
from tsspn.algorithms.ranges import NumericRange
...
class Categorical(Leaf):
    ...
    def __init__(self, p, null_value, scope, unique=None, cardinality=0):
    ...
    def copy_node(self):
        return Categorical(np.copy(self.unique), np.copy(self.p), self.null_value, self.scope, cardinality=self.cardinality)
...
class TimeSeriesLeaf(Leaf):
    # def __init__(self, time_bins, metric_bins_dict, probs_dict, scope, cardinality=0):
    #     Leaf.__init__(self, scope=scope)
    #     self.time_bins = time_bins
    #     self.metric_bins_dict = metric_bins_dict
    #     self.probs_dict = probs_dict
    #     self.cardinality = cardinality

    # def __init__(self, timestamps, metric_values, scope, cardinality=0):
    #     Leaf.__init__(self, scope=scope)
    #     self.timestamps = timestamps
    #     self.metric_values = metric_values
    #     self.cardinality = cardinality

    def __init__(self, data, scope, cardinality=0):
        Leaf.__init__(self, scope=scope)
        self.data = data
        self.cardinality = cardinality


def _interval_probability_2d(
    node, 
    left1, right1, left2, right2, 
    left1_included, right1_included, 
    left2_included, right2_included
):
    """
    Calculate the joint probability for the given 2D interval in the time series leaf node.

    :param node: TimeSeriesLeaf, the node containing sorted data and cumulative probabilities.
    :param left1: float, left bound for the first dimension (timestamp), -inf for no lower bound.
    :param right1: float, right bound for the first dimension (timestamp), +inf for no upper bound.
    :param left2: float, left bound for the second dimension (field), -inf for no lower bound.
    :param right2: float, right bound for the second dimension (field), +inf for no upper bound.
    :param left1_included: bool, whether the left bound of the first dimension is inclusive.
    :param right1_included: bool, whether the right bound of the first dimension is inclusive.
    :param left2_included: bool, whether the left bound of the second dimension is inclusive.
    :param right2_included: bool, whether the right bound of the second dimension is inclusive.
    :return: float, the joint probability of the interval.
    """
    # total_prob = 0

    # time_bins = node.time_bins
    # metric_bins_dict = node.metric_bins_dict
    # probs_dict = node.probs_dict

    # if not left1_included:
    #     left1 += 1

    # if not right1_included:
    #     right1 -= 1

    # time_start_idx = max(0, np.searchsorted(time_bins, left1, side='left') - 1)
    # time_end_idx = min(len(time_bins) - 1, np.searchsorted(time_bins, right1, side='left')) - 1

    # for i in range(time_start_idx, time_end_idx + 1):
    #     metric_bins = metric_bins_dict[i]
    #     probs = probs_dict[i]
    #     metric_start_idx = max(0, np.searchsorted(metric_bins, left2, side='left') - 1)
    #     metric_end_idx = min(len(metric_bins) - 1, np.searchsorted(metric_bins, right2, side='left')) - 1

    #     time_start, time_end = time_bins[i], time_bins[i + 1]
    #     intersect_time_start = max(left1, time_start)
    #     if left1 == -np.inf:
    #         intersect_time_start = time_start
    #     intersect_time_end = min(right1, time_end)
    #     if right1 == np.inf:
    #         intersect_time_end = time_end
    #     time_intersection = max(0, intersect_time_end - intersect_time_start)
    #     time_ratio = time_intersection / (time_end - time_start)

    #     for j in range(metric_start_idx, metric_end_idx + 1):
    #         metric_start, metric_end = metric_bins[j], metric_bins[j + 1]
    #         intersect_metric_start = max(left2, metric_start)
    #         if left2 == -np.inf:
    #             intersect_metric_start = metric_start
    #         intersect_metric_end = min(right2, metric_end)
    #         if right2 == np.inf:
    #             intersect_metric_end = metric_end
    #         metric_intersection = max(0, intersect_metric_end - intersect_metric_start)
    #         metric_ratio = metric_intersection / (metric_end - metric_start)

    #         prob = probs[j] * time_ratio * metric_ratio
    #         total_prob += prob
    ...
    timestamps = node.data[:,1]
    metric_values = node.data[:,0]

    # Handle inclusivity for the first dimension (timestamps)
    if left1_included:
        mask1 = timestamps >= left1
    else:
        mask1 = timestamps > left1

    if right1_included:
        mask1 &= timestamps <= right1
    else:
        mask1 &= timestamps < right1

    # Handle inclusivity for the second dimension (field values)
    if left2_included:
        mask2 = metric_values >= left2
    else:
        mask2 = metric_values > left2

    if right2_included:
        mask2 &= metric_values <= right2
    else:
        mask2 &= metric_values < right2

    # Combine masks for 2D interval
    mask = mask1 & mask2

    # Compute the joint probability as the proportion of points in the interval
    interval_count = np.sum(mask)

    return interval_count / timestamps.shape[0]
...
def categorical_likelihood_range(node, data, dtype=np.float64, **kwargs):
    ...
    for i, rang in enumerate(ranges):
        ...
        if node.unique is not None:
            ...
            for possible_val in rang.possible_values:
                idx = np.searchsorted(node.unique, possible_val, side='left')
                if idx < len(node.unique) and node.unique[idx] == possible_val:
                    p.append(node.p[idx])
            probs[i] = sum(p)
        else:
    ...
    end_t = perf_counter()
    node.execution_time = end_t - start_t
    return probs
...
def time_series_likelihood_range(node, data, dtype=np.float64, **kwargs):
    """
    Returns the probability for the given sets.
    """
    assert len(node.scope) == 2, node.scope

    probs = np.zeros((data.shape[0], 1), dtype=dtype)

    field_ranges = data[:, node.scope[0]]
    timestamp_ranges = data[:, node.scope[1]]

    for i, (field_range, timestamp_range) in enumerate(zip(field_ranges, timestamp_ranges)):

        # Skip if no range is specified aka use a log-probability of 0 for that instance
        if field_range is None and timestamp_range is None:
            probs[i] = 1
            continue
        elif field_range is None:
            field_range = NumericRange([[-np.inf, np.inf]])
        elif timestamp_range is None:
            timestamp_range = NumericRange([[-np.inf, np.inf]])

        for i, field_interval in enumerate(field_range.get_ranges()):
            for j, time_interval in enumerate(timestamp_range.get_ranges()):
                field_inclusive = field_range.inclusive_intervals[i]
                time_inclusive = timestamp_range.inclusive_intervals[j]
                
                probs[i] += _interval_probability_2d( \
                    node, \
                    time_interval[0], time_interval[1], \
                    field_interval[0], field_interval[1], \
                    time_inclusive[0], time_inclusive[1], \
                    field_inclusive[0], field_inclusive[1] \
                )

        if kwargs['series'] and probs[i] > 0:
            probs[i] = 1
