...
from FSPN.Inference.inference import EPSILON
import numpy as np

from spn.structure.Base import Leaf
from FSPN.Structure.StatisticalTypes import MetaType, Type
from FSPN.Structure.leaves.get_breaks import get_breaks
import logging
from FSPN.Structure.leaves.fspn_leaves.dimension_reduction import PCA_reduction
from FSPN.Structure.leaves.fspn_leaves.utils import discretize_series
...
class Multi_histogram(Leaf):
    ...
    def query(self, query, attr):
        ...
        if type(query) == tuple:
            ...
            if self.red_machine:
                probs = self.infer_range_query_with_reduction(query)
            else:
                probs = self.infer_range_query(query)
        else:
            if set(attr) != set(self.scope):
                query_idx = [attr.index(i) for i in self.scope]
                query = query[:, query_idx]
            probs = self.infer_point_query(query)
        end_t = perf_counter()
        self.execution_time = end_t - start_t
        return probs
    ...
    if pca:
        ...
        for i, s in enumerate(scope):
            ...
            if meta_type == MetaType.BINARY or meta_type == MetaType.DISCRETE or discretize == False:
                ...
                # if len(unique_vals) > MAX_UNIQUE_LEAF_VALUES:
                #     _, cont_break = np.histogram(data[:, i], bins=MAX_UNIQUE_LEAF_VALUES, density=False)
                # else:
                #     cont_break = unique_vals
