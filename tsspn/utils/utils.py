...
def parse_query(query_str, schema):
    ...
    """
    query = Query(schema)
    query_str = query_str.replace("host = ", "_host = ")
    query_str = query_str.replace("instance = ", "_instance = ")
    ...
    projection_attributes = [token for token in tokens_before_from if isinstance(token, sqlparse.sql.Identifier)]

    if len(count_statements) == 1 or len(projection_attributes) == 1:
...
def get_evaluation_results():
    """
    Collects and combines evaluation results from all estimators across all datasets.
    
    Returns:
        pandas.DataFrame: Combined dataframe with columns like:
            - query_no: Query identifier
            - query: SQL query string
            - real_points: Actual cardinality for points
            - pred_points: Predicted cardinality for points
            - points_qerror: Q-error for points prediction
            - points_infer_time: Inference time for points prediction
            - real_series: Actual cardinality for series
            - pred_series: Predicted cardinality for series
            - series_qerror: Q-error for series prediction
            - series_infer_time: Inference time for series prediction
            - estimator: Name of the estimator
            - dataset: Name of the dataset
    """
    # estimators = ["tsspn", "deepdb", "mscn", "fspn", "naru", "timescale", "sampling"]
    estimators = ["tsspn", "deepdb", "MaxDiff", "timescale", "sampling", "mscn"]
    df_list = []
    for schema in ALL_TABLE_SCHEMAS:
        for estimator in estimators:
            result_file = os.path.join(RESULTS_DIR_PATH, estimator, f"{schema.table_name}_realworld.csv")
            df = pd.read_csv(result_file, header=0).assign(estimator=estimator).assign(dataset=schema.dataset)
            df_list.append(df)
...
# 所有的 estimator 名称，固定顺序
ESTIMATORS = ["Timescale", "Sample", "MaxDiff", "DeepDB", "MSCN", "FISTS"]