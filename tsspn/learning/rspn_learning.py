...
from tsspn.learning.tsspn_learning import get_split_rows_by_one_column
from tsspn.structure.base import Sum, Product
from tsspn.structure.hypercube import create_hypercube_leaf
from tsspn.structure.discrete_multi_histogram import create_discrete_multi_histogram_leaf
...
def build_rspn(
    ...
    next_operation = get_next_operation(min_instances_slice, min_features_slice, multivariate_leaf)
    split_rows_by_one_column = get_split_rows_by_one_column(min_instances_slice)
    ...
    while tasks:
        ...
        logging.debug("OP: {} on slice {}, scope {} (remaining tasks {})".format(operation, local_data.shape, scope, len(tasks)))
        ...
        if operation == Operation.REMOVE_UNINFORMATIVE_FEATURES:
            ...
            split_start_t = perf_counter()
            if build_mh:
                data_slices = [(local_data, scope, 1)]
            else:
            ...
            if len(data_slices) == 1:
                if build_mh or len(scope) <= 2:
                    start_t = perf_counter()
                    node = create_discrete_multi_histogram_leaf(local_data, scope)
                    end_t = perf_counter()
                    logging.debug(
                        "\t\tcreated multi histogram leaf (in {:.5f} secs)".format(end_t - start_t)
                    )
                    parent.children[children_pos] = node
                    continue
                else:
                    # data_slices = [(local_data[:, :2], scope[:2], 1)]
                    # for i in range(2, len(scope)):
                    #     data_slices.append((local_data[:, i].reshape(-1,1), [scope[i]], 1))
                    data_slices = []
                    for i in range(len(scope)):
                        data_slices.append((local_data[:, i].reshape(-1, 1), [scope[i]], 1))
                    logging.debug(
                        "\t\tmanually split {} col clusters".format(len(data_slices))
                    )

                # split_start_t = perf_counter()
                # data_slices = split_rows_by_one_column(local_data, scope)
                # if len(data_slices) == 1:
                ...
                # split_end_t = perf_counter()
                # logging.debug(
                #     "\t\tfound {} row clusters by one column (in {:.5f} secs)".format(len(data_slices), split_end_t - split_start_t)
                # )

                # create_sum_node(children_pos, data_slices, parent, scope, tasks)
                # continue
            ...
            for data_slice, scope_slice, _ in data_slices:
                ...
                logging.debug(
                    "\t\tindependent scope {}".format(scope_slice)
                )
                node.children.append(None)
...
def create_sum_node(children_pos, data_slices, parent, scope, tasks, cluster_centers=None):
    ...
    for data_slice, scope_slice, proportion in data_slices:
        assert isinstance(scope_slice, list), "slice must be a list"
        logging.debug(
            "\t\tslices size {}, proportion {}".format(len(data_slice), proportion)
        )
